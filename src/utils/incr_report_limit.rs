...
pub async fn incr_report_limit(redis_pool: &RedisPool, ip: &str) -> anyhow::Result<()> {
    ...
    increx
        ...
        .await
        .map_err(|error| anyhow!("unable to increment the report limit: {error:?}"))
}
...
mod tests {
    ...
    mod serial {
        ...
        #[tokio::test]
        async fn can_increment_report_limit_for_a_missing_key(ctx: &mut RedisTestContext) {
        ...
        #[test_context(RedisTestContext)]
        #[tokio::test]
        async fn can_increment_report_limit_for_an_existing_key(ctx: &mut RedisTestContext) {
            let redis_pool = &ctx.redis_pool;
            let mut conn = redis_pool.get().await.unwrap();
