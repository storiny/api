...
/// Returns the `published_story_count` for a blog.
#[tracing::instrument(
    name = "GRPC get_blog_published_story_count",
    skip_all,
    fields(
...
"#,
    ...
    Ok(Response::new(GetBlogPublishedStoryCountResponse {
...
mod tests {
    use crate::{
        grpc::defs::blog_def::v1::GetBlogPublishedStoryCountRequest,
        test_utils::test_grpc_service,
    };
    use sqlx::PgPool;
    use tonic::Request;

    #[sqlx::test(fixtures("get_blog_published_story_count"))]
    async fn can_return_blog_published_story_count_by_slug(pool: PgPool) {
        test_grpc_service(
            pool,
            false,
            Box::new(|mut client, _, _, _| async move {
                let response = client
                    .get_blog_published_story_count(Request::new(
                        GetBlogPublishedStoryCountRequest {
                ...
                assert_eq!(response.published_story_count, 3_u32);
            }),
        )
        .await;
    }
    ...
    #[sqlx::test(fixtures("get_blog_published_story_count"))]
    async fn can_return_blog_published_story_count_by_domain(pool: PgPool) {
        test_grpc_service(
            pool,
            false,
            Box::new(|mut client, _, _, _| async move {
    ...
    #[sqlx::test(fixtures("get_blog_published_story_count"))]
    async fn can_return_blog_published_story_count_by_id(pool: PgPool) {
        test_grpc_service(
            pool,
            false,
            Box::new(|mut client, _, _, _| async move {
    ...
    #[sqlx::test(fixtures("get_blog_published_story_count"))]
    async fn should_not_count_soft_deleted_published_stories(pool: PgPool) {
        test_grpc_service(
            pool,
            false,
            Box::new(|mut client, pool, _, _| async move {
                // Should count all the published stories initially.
                let response = client
                    .get_blog_published_story_count(Request::new(
                        GetBlogPublishedStoryCountRequest {
                            identifier: "test-blog".to_string(),
...
"#,
                ...
                // Should count only valid published stories.
                let response = client
                    .get_blog_published_story_count(Request::new(
                        GetBlogPublishedStoryCountRequest {
                            identifier: "test-blog".to_string(),
