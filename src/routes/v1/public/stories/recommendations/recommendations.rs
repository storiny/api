...
#[get("/v1/public/stories/{story_id}/recommendations")]
#[tracing::instrument(
...
async fn get(
    ...
    if let Some(user_id) = user_id {
        let result = sqlx::query_file_as!(
            Story,
            "queries/public/recommendations/logged_in.sql",
            story_id,
            10 as i16,
            (page * 10) as i16,
            user_id
        ...
        let result = sqlx::query_file_as!(
            Story,
            "queries/public/recommendations/default.sql",
            story_id,
            10 as i16,
            (page * 10) as i16
        )
...
mod tests {
    ...
    #[sqlx::test(fixtures("recommendation"))]
    async fn should_not_include_soft_deleted_stories_in_recommendations(
        pool: PgPool,
    ) -> sqlx::Result<()> {
...
"#,
    ...
    #[sqlx::test(fixtures("recommendation"))]
    async fn should_not_include_unpublished_stories_in_recommendations(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let app = init_app_for_test(get, pool, false, false, None).await.0;
...
"#,
    ...
    #[sqlx::test(fixtures("recommendation"))]
    async fn can_return_is_liked_flag_for_recommended_stories_when_logged_in(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
...
"#,
    ...
    #[sqlx::test(fixtures("recommendation"))]
    async fn can_return_is_bookmarked_flag_for_recommended_stories_when_logged_in(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(get, pool, true, true, Some(1_i64)).await;
...
"#,
    ...
    #[sqlx::test(fixtures("recommendation"))]
    async fn should_not_include_soft_deleted_stories_in_recommendations_when_logged_in(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, _) = init_app_for_test(get, pool, true, true, Some(1_i64)).await;
...
"#,
    ...
    #[sqlx::test(fixtures("recommendation"))]
    async fn should_not_include_unpublished_stories_in_recommendations_when_logged_in(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, _) = init_app_for_test(get, pool, true, true, Some(1_i64)).await;
