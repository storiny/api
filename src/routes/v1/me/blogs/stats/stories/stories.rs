...
#[get("/v1/me/blogs/{blog_id}/stats/stories")]
#[tracing::instrument(
    name = "GET /v1/me/blogs/{blog_id}/stats/stories",
    skip_all,
    fields(
        user_id = user.id().ok(),
...
returning_readers AS (
    ...
    FROM (
        SELECT FROM story_reads sr
            INNER JOIN blog_stories bs
                ON bs.story_id = sr.story_id
                AND bs.blog_id = $1
                AND bs.accepted_at IS NOT NULL
                AND bs.deleted_at IS NULL
        WHERE
...
"#,
    ...
    builder.append_header((header::VARY, "x-storiny-blog-id"));
...
mod tests {
    ...
    async fn can_return_blog_stories_stats_as_blog_owner(pool: PgPool) -> sqlx::Result<()> {
        let (app, cookie, _) = init_app_for_test(get, pool, true, true, Some(1_i64)).await;

        let req = test::TestRequest::get()
            .cookie(cookie.unwrap())
            .uri(&format!("/v1/me/blogs/{}/stats/stories", 4))
            .to_request();
    ...
    #[sqlx::test(fixtures("stories"))]
    async fn can_return_blog_stories_stats_as_blog_editor(pool: PgPool) -> sqlx::Result<()> {
...
"#,
    ...
    #[sqlx::test]
    async fn can_return_blog_stories_stats_for_a_minimal_blog(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(get, pool, true, false, None).await;
