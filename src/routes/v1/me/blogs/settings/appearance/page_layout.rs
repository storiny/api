...
#[patch("/v1/me/blogs/{blog_id}/settings/appearance/page-layout")]
#[tracing::instrument(
    name = "PATCH /v1/me/blogs/{blog_id}/settings/appearance/page-layout",
    skip_all,
    fields(
        user = user.id().ok(),
...
UPDATE blogs
SET is_homepage_large_layout = $3
WHERE
    id = $2
...
mod tests {
    ...
    #[sqlx::test]
    async fn can_update_page_layout_settings_as_blog_owner(pool: PgPool) -> sqlx::Result<()> {
...
VALUES ($1, $2, $3)
RETURNING id, is_homepage_large_layout
"#,
        )
        ...
        assert!(!result.get::<bool, _>("is_homepage_large_layout"));

        let req = test::TestRequest::patch()
            .cookie(cookie.unwrap())
            .uri(&format!(
                "/v1/me/blogs/{blog_id}/settings/appearance/page-layout",
            ))
            .set_json(Request {
        ...
        let result = sqlx::query(
            r#"
SELECT is_homepage_large_layout FROM blogs
WHERE id = $1
"#,
        )
        .bind(blog_id)
    ...
    #[sqlx::test]
    async fn can_update_page_layout_settings_as_blog_editor(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_page_layout_settings_request_for_a_deleted_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
