...
#[patch("/v1/me/blogs/{blog_id}/settings/appearance/branding")]
#[tracing::instrument(
    name = "PATCH /v1/me/blogs/{blog_id}/settings/appearance/branding",
    skip_all,
    fields(
        user = user.id().ok(),
...
"#,
    ...
    .bind(user_id)
    .bind(blog_id)
    .bind(payload.hide_storiny_branding)
...
mod tests {
    ...
    #[sqlx::test]
    async fn can_update_branding_settings_as_blog_owner(pool: PgPool) -> sqlx::Result<()> {
...
VALUES ($1, $2, $3, TRUE)
RETURNING id, hide_storiny_branding
"#,
        )
        ...
        assert!(!result.get::<bool, _>("hide_storiny_branding"));
        ...
        let result = sqlx::query(
            r#"
SELECT hide_storiny_branding FROM blogs
WHERE id = $1
"#,
        )
        .bind(blog_id)
    ...
    #[sqlx::test]
    async fn can_update_branding_settings_as_blog_editor(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_branding_settings_request_for_a_regular_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
