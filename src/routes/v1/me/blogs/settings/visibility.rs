...
#[patch("/v1/me/blogs/{blog_id}/settings/visibility")]
#[tracing::instrument(
    name = "PATCH /v1/me/blogs/{blog_id}/settings/visibility",
    skip_all,
    fields(
        user = user.id().ok(),
...
mod tests {
    ...
    #[sqlx::test]
    async fn can_update_visibility_settings_as_blog_owner(pool: PgPool) -> sqlx::Result<()> {
...
"#,
        ...
        assert!(!result.get::<bool, _>("is_external"));

        let req = test::TestRequest::patch()
            .cookie(cookie.unwrap())
            .uri(&format!("/v1/me/blogs/{blog_id}/settings/visibility"))
...
"#,
    ...
    #[sqlx::test]
    async fn can_update_visibility_settings_as_blog_editor(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_visibility_settings_request_for_a_regular_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_visibility_settings_request_for_a_deleted_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
