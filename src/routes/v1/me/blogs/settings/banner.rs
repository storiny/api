...
#[patch("/v1/me/blogs/{blog_id}/settings/banner")]
#[tracing::instrument(
    name = "PATCH /v1/me/blogs/{blog_id}/settings/banner",
    skip_all,
    fields(
        user = user.id().ok(),
...
mod tests {
    ...
    #[sqlx::test]
    async fn can_set_a_banner_as_blog_owner(pool: PgPool) -> sqlx::Result<()> {
...
"#,
        ...
        let req = test::TestRequest::patch()
            ...
            .uri(&format!("/v1/me/blogs/{blog_id}/settings/banner"))
            .set_json(Request {
        ...
        let result = sqlx::query(
            r#"
SELECT banner_id, banner_hex FROM blogs
WHERE id = $1
"#,
        )
        .bind(blog_id)
    ...
    #[sqlx::test]
    async fn can_set_a_banner_as_blog_editor(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
        let banner_id = Uuid::new_v4();
...
"#,
        ...
        // Insert a blog with banner.
        let result = sqlx::query(
            r#"
INSERT INTO blogs (name, slug, user_id, banner_id, banner_hex, has_plus_features)
VALUES ($1, $2, $3, $4, $5, TRUE)
RETURNING id, banner_id, banner_hex
"#,
        )
        .bind("Sample blog".to_string())
        .bind("sample-blog".to_string())
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_a_banner_update_request_for_a_regular_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_a_banner_update_request_for_a_deleted_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(patch, pool, true, false, None).await;
