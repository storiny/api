...
#[get("/v1/me/blogs/{blog_id}/content/published-stories")]
#[tracing::instrument(
    name = "GET /v1/me/blogs/{blog_id}/content/published-stories",
    skip_all,
    fields(
        user_id = user.id().ok(),
...
"#,
    ...
    if has_search_query {
        let result = sqlx::query_file_as!(
            Story,
            "queries/me/blogs/published_stories/with_query.sql",
            search_query,
            blog_id,
            user_id,
            10 as i16,
        ...
        let result = sqlx::query_file_as!(
            Story,
            "queries/me/blogs/published_stories/default_asc.sql",
            blog_id,
            user_id,
            10 as i16,
            (page * 10) as i16
        ...
        let result = sqlx::query_file_as!(
            Story,
            "queries/me/blogs/published_stories/default_desc.sql",
            blog_id,
            user_id,
            10 as i16,
            (page * 10) as i16
...
mod tests {
    ...
    #[sqlx::test(fixtures("published_story"))]
    async fn can_return_published_stories(pool: PgPool) -> sqlx::Result<()> {
        ...
        let req = test::TestRequest::get()
            ...
            .uri(&format!("/v1/me/blogs/{}/content/published-stories", 3_i64))
            .to_request();
    ...
    #[sqlx::test(fixtures("published_story"))]
    async fn can_return_published_stories_for_blog_owner(pool: PgPool) -> sqlx::Result<()> {
...
"#,
    ...
    #[sqlx::test(fixtures("published_story"))]
    async fn can_return_published_stories_for_blog_editor(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(get, pool, true, true, Some(1_i64)).await;
...
"#,
    ...
    async fn can_return_published_stories_in_asc_order(pool: PgPool) -> sqlx::Result<()> {
        ...
        let req = test::TestRequest::get()
            .cookie(cookie.unwrap())
            .uri(&format!(
                "/v1/me/blogs/{}/content/published-stories?sort=old",
                3_i64
            ))
            .to_request();
        let res = test::call_service(&app, req).await;
    ...
    async fn can_return_published_stories_in_desc_order(pool: PgPool) -> sqlx::Result<()> {
        ...
        let req = test::TestRequest::get()
            .cookie(cookie.unwrap())
            .uri(&format!(
                "/v1/me/blogs/{}/content/published-stories?sort=recent",
                3_i64
            ))
            .to_request();
        let res = test::call_service(&app, req).await;
    ...
    async fn can_search_published_stories(pool: PgPool) -> sqlx::Result<()> {
        ...
        let req = test::TestRequest::get()
            .cookie(cookie.unwrap())
            .uri(&format!(
                "/v1/me/blogs/{}/content/published-stories?query={}",
                3_i64,
                encode("two")
            ))
            .to_request();
