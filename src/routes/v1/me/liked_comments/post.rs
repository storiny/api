...
#[tracing::instrument(
    name = "POST /v1/me/liked-comments/{comment_id}",
    skip_all,
    fields(
        user_id = user.id().ok(),
...
async fn post(
    ...
    if !check_resource_limit(&data.redis, ResourceLimit::LikeComment, user_id).await? {
...
"#,
    ...
    {
        Ok(_) => {
            incr_resource_limit(&data.redis, ResourceLimit::LikeComment, user_id).await?;
        ...
        Err(error) => {
            if let Some(db_err) = error.as_database_error() {
                ...
                // Do not throw if the comment is already liked.
                ...
                if matches!(error_kind, sqlx::error::ErrorKind::ForeignKeyViolation) {
                    return Err(AppError::from("Comment does not exist"));
                }
                ...
                if error_code == SqlState::EntityUnavailable.to_string() {
                    return Err(AppError::from("Comment being liked is deleted"));
                }
            }
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_a_comment_like_request_for_a_missing_comment(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let (app, cookie, _) = init_app_for_test(post, pool, true, false, None).await;
...
"#,
        ...
        #[test_context(RedisTestContext)]
        #[sqlx::test]
        async fn can_reject_a_comment_like_request_on_exceeding_the_resource_limit(
