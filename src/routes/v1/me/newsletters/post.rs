...
#[post("/v1/me/newsletters/{blog_id}")]
#[tracing::instrument(
    name = "POST /v1/me/newsletters/{blog_id}",
    skip_all,
    fields(
        user_id = user.id().ok(),
...
async fn post(
    ...
    if !check_resource_limit(&data.redis, ResourceLimit::SubscribeRegistered, user_id).await? {
        return Err(AppError::new_client_error_with_status(
            StatusCode::TOO_MANY_REQUESTS,
            "Daily subscription limit exceeded. Try again tomorrow.",
        ));
    }
...
"#,
    ...
    {
        Ok(_) => {
            incr_resource_limit(&data.redis, ResourceLimit::SubscribeRegistered, user_id).await?;
        ...
        Err(error) => {
            if let Some(db_err) = error.as_database_error() {
                ...
                // Do not throw if the user has already subscribed.
...
mod tests {
    ...
    #[sqlx::test]
    async fn can_reject_a_subscription_request_for_a_missing_blog(
        pool: PgPool,
    ) -> sqlx::Result<()> {
    ...
    mod serial {
        ...
        async fn can_subscribe_to_a_newsletter(
            ...
            // Subscriber should be present in the database.
...
"#,
            ...
            let result = get_resource_limit(
                &ctx.redis_pool,
                ResourceLimit::SubscribeRegistered,
                user_id.unwrap(),
            )
            .await;
        ...
        #[test_context(RedisTestContext)]
        #[sqlx::test]
        async fn can_reject_a_subscription_request_on_exceeding_the_resource_limit(
            ctx: &mut RedisTestContext,
            pool: PgPool,
        ) -> sqlx::Result<()> {
            let (app, cookie, user_id) = init_app_for_test(post, pool, true, false, None).await;
        ...
        #[test_context(RedisTestContext)]
        #[sqlx::test(fixtures("newsletter"))]
        async fn should_not_throw_when_subscribing_to_an_already_subscribed_newsletter(
