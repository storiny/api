...
async fn handle_dribbble_oauth_request(
...
/// Handles Dribbble profile response and saves the connection to the database.
///
/// * `dribbble_data` - The Dribbble profile endpoint response.
/// * `data` - The shared app state.
/// * `user_id` - The ID of the user who requested this flow.
#[tracing::instrument(skip_all, fields(user_id), err)]
async fn handle_dribbble_data(
    dribbble_data: Response,
    data: &web::Data<AppState>,
    user_id: &i64,
) -> Result<(), ConnectionError> {
    let provider_identifier = dribbble_data.login;
    let display_name = dribbble_data.name;
...
#[tracing::instrument(
    name = "GET /oauth/dribbble/callback",
    skip_all,
...
mod tests {
    ...
    #[get("/connect-dribbble-account")]
    async fn get(data: web::Data<AppState>, user: Identity) -> impl Responder {
        let user_id = user.id().unwrap();

        match handle_dribbble_data(
            Response {
                login: "dribbble_user".to_string(),
                name: "Dribbble user".to_string(),
            },
            &data,
            &user_id,
        )
    ...
    #[sqlx::test]
    async fn can_connect_a_dribbble_account(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(get, pool, true, false, None).await;

        let req = test::TestRequest::get()
            .cookie(cookie.unwrap())
            .uri("/connect-dribbble-account")
            .to_request();
...
"#,
    ...
    #[sqlx::test]
    async fn can_reject_connecting_a_duplicate_dribbble_account(pool: PgPool) -> sqlx::Result<()> {
        let mut conn = pool.acquire().await?;
        let (app, cookie, user_id) = init_app_for_test(get, pool, true, false, None).await;
