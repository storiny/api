...
impl Encode for AwarenessUpdate {
    fn encode<E: Encoder>(&self, encoder: &mut E) {
        ...
        for (&client_id, entry) in self.clients.iter() {
            encoder.write_var(client_id);
            encoder.write_var(entry.clock);
            encoder.write_string(&entry.json);
        }
    }
}
...
impl MetaClientState {
    /// Creates a new [MetaClientState] instance.
    ///
    /// * `clock` - The client's clock value.
    /// * `last_updated` - The instant when the client was last updated.
...
impl Event {
    ...
    ///
    /// * `added` - The added clients.
    /// * `updated` - The updated clients.
    /// * `removed` - The removed clients.
...
/// The Awareness class implements a shared state protocol that is used for non-persistent data
/// like awareness information (cursor, username). Each client can update its own local state and
/// listen to state changes of remote clients.
///
/// Each client is identified by a unique client id (something we borrow from `doc.clientID`). A
/// client can override its own state by propagating a message with an increasing timestamp
...
impl Awareness {
    ...
    /// Returns a read-write reference to the underlying [Doc].
    pub fn doc_mut(&mut self) -> &mut Doc {
        &mut self.doc
    }

    /// Returns a globally unique client ID of the underlying [Doc].
    ...
    /// Clears out the state of a given client, effectively marking it as disconnected.
    ///
    ...
    /// Clears out a state of a current client (see: [Awareness::client_id]), effectively marking it
    /// as disconnected.
    ...
    /// Updates a client's meta.
    ///
    /// * `client_id` - The unique ID of the client.
    fn update_meta(&mut self, client_id: ClientID) {
        match self.meta.entry(client_id) {
            Entry::Occupied(mut entry) => {
                let clock = entry.get().clock + 1;
                let meta = MetaClientState::new(clock, Instant::now());
                entry.insert(meta);
            }
            Entry::Vacant(entry) => {
                entry.insert(MetaClientState::new(1, Instant::now()));
            }
        }
    }

    /// Returns a serializable update object which is representation of the current awareness state.
    ...
    /// Returns a serializable update object which is representation of the current awareness state.
    /// Unlike [Awareness::update], this method variant allows us to prepare update only for a
    /// subset of known clients. These clients must all be known to the current [Awareness]
                ...
                Entry::Occupied(mut e) => {
                    ...
                    if is_new || is_removed {
                        if is_null {
                            ...
                            {
                                // Remote client removed the local state. Broadcast a message
...
mod test {
    ...
    fn update(
        ...
        let update =
            from.update_with_clients([event.added(), event.updated(), event.removed()].concat())?;
